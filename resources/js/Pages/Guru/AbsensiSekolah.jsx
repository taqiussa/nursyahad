...
const AbsensiSekolah = ({ initTahun, listKehadiran, listKelas }) => {
    ...
    async function getDataAbsensiSiswa() {
        const response = await getAbsensiSekolah(data.tanggal, data.tahun, data.jam, data.kelasId)
        setListSiswa(response.listSiswa)
    }

    async function getDataInfoAbsensi() {
        const response = await getInfoAbsensiSekolah(data.tanggal, data.kelasId)
    ...
    const handleDynamic = (e, index, id, nis, name, guruName) => {
        ...
        axios.post(route('absensi-sekolah.simpan',
            {
                id: id,
    ...
    return (
        <>
            <Head title='Absensi Sekolah' />
            <div className="font-bold text-lg text-center text-slate-600 uppercase border-b-2 border-emerald-500 mb-3 bg-emerald-200">absensi sekolah</div>
                                ...
                                <td className="py-2 px-2 font-medium text-slate-600">
                                    <div className='flex flex-col'>
                                        ...
                                        <Kehadiran
                                            ...
                                            onChange={(e) => handleDynamic(e, index, siswa.absensi?.id, siswa.nis, siswa.user.name, siswa.absensi.guru?.name)}
...
AbsensiSekolah.layout = page => <AppLayout children={page} />
export default AbsensiSekolah